...
apt update -qq
apt install -y -qq nano sudo syslog-ng isc-dhcp-server
...
filter f_all { netmask ("10.$X.$Y.10/255.255.255.0"); };
log { source(s_net); filter(f_dhcp); filter(f_all); destination(d_esckere); };
EOF
